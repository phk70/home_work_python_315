...
Колесников Павел.
HW-11 от 13.02.2024
Города. Аннотация типов + MYPY
"""
"""
К функциям надо добавить аннотацию типов, с использованием Typing.
Проверка Mypy должна проходить успешно. Все функции должны иметь аннотацию типов.
Старайтесь свести использование Any к минимуму. Все же мы знаем, какие там типы данных наверняка.

Критерии проверки:
Сдаем работу в виде архива (код + скриншот с пройденной проверкой mypy + cities.json ).
Дата-сет читается из файла json со списком городов.
Вся игра описана функциями.
Каждая функция отвечает за какое-то одно действие игры.
Запускает всю игру функция main().
Хороший нейминг переменных и функций.
Документация для функций.
Аннотация типов с Typing.
Проверка mypy проходит.
***************************************************************************
...
create_count('count.json') для создания файла со счетом
create_cities_set('cities.json') для создания файла с городами
...
Создает и записывает чистый список городов без плохих букв в '***.json'
    ...
    for city in list(cities_set):
        if city[-1] in list(bad_letters_set):
            ...
            cities_set: List[Any] = list(cities_set)
    # Записываем в JSON
    with open(name_file, 'w', encoding='UTF-8') as file:
        json.dump(cities_set, file, ensure_ascii=False, indent=4)


def save_data_in_file(name: Dict[str, str], name_file: str) -> None:
    """
...
def load_data_from_file(name: Dict[str, str], name_file: str) -> Dict[str, str]:
    """
...
def create_count(name_file: str):
    """
...
def up_count(name, key, point: int) -> str:
    """
...
def main():
    ...
    *******************************************************************************************''')
    ...
    while True:
        # ************************************** Ход игрока *********************************************
        # ***********************************************************************************************
        user_city: str = input('Введите город: ').strip().title()
        ...
        last_user_letter: str = user_city[-1]
